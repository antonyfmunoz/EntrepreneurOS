...
function Get-FlySecrets([string]$App) {
  $raw = cmd.exe /d /s /c "flyctl secrets list --app $App --json 2>NUL"
...
$required = @(
  ...
  "EOS_ALERT_EMAIL_SENDER_USER_ID",
  "EOS_ALERT_EMAIL_SENDER_ADDRESS",
  "EOS_ALERT_EMAIL_RECIPIENT",
  ...
  "EOS_ARTIFACT_S3_FORCE_PATH_STYLE",
  ...
  "EOS_ARTIFACT_BACKUP_S3_FORCE_PATH_STYLE",
...
function Import-FlySecretsFromEnvironment([string]$App, [string[]]$Names) {
...
try {
  ...
  try {
    ...
    npm run db:migrate
    if ($LASTEXITCODE -ne 0) { throw "Production schema migration failed before credential cutover." }
  } finally {
    $env:DATABASE_URL = $runtimeDatabaseUrl
  }
  npm run db:migrations:verify
  if ($LASTEXITCODE -ne 0) { throw "The least-privilege runtime role cannot verify the exact production migration state." }

  $runtimeSecretNames = @(
    "ANTHROPIC_API_KEY", "CLERK_PUBLISHABLE_KEY", "CLERK_SECRET_KEY", "DATABASE_URL", "SESSION_SECRET",
    "EOS_CREDENTIAL_ENCRYPTION_KEY", "EOS_PUBLIC_ORIGIN", "EOS_ALERT_WEBHOOK_URL", "EOS_ALERT_WEBHOOK_SECRET",
    ...
    "EOS_ARTIFACT_STORAGE_PROVIDER", "EOS_ARTIFACT_S3_BUCKET", "EOS_ARTIFACT_S3_REGION", "EOS_ARTIFACT_S3_ENDPOINT",
    "EOS_ARTIFACT_S3_FORCE_PATH_STYLE", "EOS_ARTIFACT_S3_PREFIX", "EOS_ARTIFACT_S3_SSE_CUSTOMER_KEY",
    "EOS_ARTIFACT_S3_ACCESS_KEY_ID", "EOS_ARTIFACT_S3_SECRET_ACCESS_KEY", "EOS_ARTIFACT_BACKUP_STORAGE_PROVIDER",
    "EOS_ARTIFACT_BACKUP_S3_BUCKET", "EOS_ARTIFACT_BACKUP_S3_REGION", "EOS_ARTIFACT_BACKUP_S3_ENDPOINT",
    "EOS_ARTIFACT_BACKUP_S3_FORCE_PATH_STYLE", "EOS_ARTIFACT_BACKUP_S3_PREFIX", "EOS_ARTIFACT_BACKUP_S3_SSE_CUSTOMER_KEY",
    "EOS_ARTIFACT_BACKUP_S3_ACCESS_KEY_ID", "EOS_ARTIFACT_BACKUP_S3_SECRET_ACCESS_KEY",
    "EOS_CANDIDATE_STT_ENABLED", "EOS_CANDIDATE_STT_MODEL", "EOS_UNTRUSTED_UPLOADS_ENABLED", "POSTHOG_API_KEY",
    "GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "GOOGLE_REDIRECT_URI", "NOTION_CLIENT_ID", "NOTION_CLIENT_SECRET",
    "NOTION_REDIRECT_URI", "SLACK_CLIENT_ID", "SLACK_CLIENT_SECRET", "SLACK_REDIRECT_URI"
  )
  ...
  if ($env:EOS_CANDIDATE_STT_ENABLED -eq "true") {
    Import-FlySecretsFromEnvironment -App $app -Names @("OPENAI_API_KEY")
  }
