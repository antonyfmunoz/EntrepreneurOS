...
export type ControlDefinition = { key: string; allowedScopes: EvidenceScope[]; maximumAgeDays: number };
export type ControlLayerDefinition = { layer: number; name: string; controls: ControlDefinition[] };

const repository = (key: string, maximumAgeDays = 30): ControlDefinition => ({ key, allowedScopes: ["repository", "production"], maximumAgeDays });
const production = (key: string, maximumAgeDays = 90): ControlDefinition => ({ key, allowedScopes: ["production"], maximumAgeDays });
const professional = (key: string, maximumAgeDays = 365): ControlDefinition => ({ key, allowedScopes: ["professional"], maximumAgeDays });
...
export const CONTROL_LAYERS: ControlLayerDefinition[] = [
  ...
  { layer: 3, name: "Database and storage", controls: [production("database_migration_and_storage", 30)] },
  { layer: 4, name: "Authentication and permissions", controls: [production("identity_acceptance")] },
  { layer: 5, name: "Hosting and deployment", controls: [production("deployment_smoke", 30)] },
  ...
  { layer: 18, name: "Cost controls", controls: [production("ai_cost_policy")] },
  ...
  { layer: 24, name: "Release and experience quality", controls: [production("accessibility_performance_release", 30), production("release_owner_approval", 7)] },
];

export const CONTROL_DEFINITIONS = new Map(CONTROL_LAYERS.flatMap((layer) => layer.controls).map((control) => [control.key, control]));

export function controlEvidenceIsCurrent(input: { definition: ControlDefinition; evidenceScope: string; reviewedAt: Date; expiresAt: Date | null; now?: Date }): boolean {
