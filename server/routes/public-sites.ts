...
const publicSiteRateLimit = fixedWindowRateLimit({
  limit: 60,
  windowMs: 60_000,
  namespace: "eos-public-sites",
});
const pathSchema = z
  .string()
  .trim()
...
const publicPageSchema = z
  .object({
    ...
    path: pathSchema,
    sections: nativeSiteSectionsSchema,
  })
  .passthrough();
const publicSiteSchema = z
  .object({ brandName: z.string().trim().min(2).max(160) })
  .passthrough();
const publicCaptureFormSchema = z
  .object({ publicCapture: z.literal(true) })
  .passthrough();
...
async function resolvedCtaHref(
  page: typeof eosInstrumentObjects.$inferSelect,
  definition: z.infer<typeof publicPageSchema>,
) {
  if (!definition.primaryCtaLabel) return "";
  if (definition.primaryCtaTarget === "manual")
    return definition.primaryCtaHref;
  ...
  const [funnel] = await db
    ...
    .limit(1);
  return funnel && publicFunnelSchema.safeParse(funnel.data).success
    ? `/f/${funnel.id}`
    : "";
}
function pagePayload(
  ...
  return {
    schemaVersion: "eos.public-page.v2",
    page: {
      id: page.id,
      siteName,
      companyName,
    ...
    try {
      ...
      if (error instanceof ZodError)
        return res.status(404).json({
          code: "public_page_unavailable",
          message: "This EOS page is unavailable.",
        });
      next(error);
    }
  };
  ...
  app.get(
    ...
    route(async (req, res) => {
      ...
      const [page] = await db
        ...
        .where(
          and(
            eq(eosInstrumentObjects.id, pageId),
            eq(eosInstrumentObjects.instrumentKey, "websites"),
            eq(eosInstrumentObjects.objectType, "page"),
            eq(eosInstrumentObjects.state, "active"),
          ),
        )
        .limit(1);
      const definition = publicPageSchema.safeParse(page?.data);
      if (!page || !definition.success) {
        res.status(404).json({
          code: "public_page_unavailable",
          message: "This EOS page is unavailable.",
        });
      ...
      const [site] = await db
        ...
        .where(
          and(
            eq(eosInstrumentObjects.id, definition.data.siteObjectId),
            eq(eosInstrumentObjects.companyId, page.companyId),
            eq(eosInstrumentObjects.instrumentKey, "websites"),
            eq(eosInstrumentObjects.objectType, "site"),
            eq(eosInstrumentObjects.state, "active"),
          ),
        )
        .limit(1);
      const siteData = publicSiteSchema.safeParse(site?.data);
      if (!site || !siteData.success) {
        res.status(404).json({
          code: "public_site_unavailable",
          message: "This EOS site is unavailable.",
        });
        return;
      }
      ...
      res.json(
        pagePayload(
          page,
          siteData.data.brandName,
          company?.name || "Organization",
          definition.data,
          await resolvedCtaHref(page, definition.data),
        ),
      );
    }),
  );
  ...
  app.get(
    ...
    route(async (req, res) => {
      ...
      const [site] = await db
        ...
        .where(
          and(
            eq(eosInstrumentObjects.id, siteId),
            eq(eosInstrumentObjects.instrumentKey, "websites"),
            eq(eosInstrumentObjects.objectType, "site"),
            eq(eosInstrumentObjects.state, "active"),
          ),
      ...
      const candidates = await db
        ...
        .where(
          and(
            eq(eosInstrumentObjects.companyId, site.companyId),
            eq(eosInstrumentObjects.instrumentKey, "websites"),
            eq(eosInstrumentObjects.objectType, "page"),
            eq(eosInstrumentObjects.state, "active"),
          ),
      ...
      const page = candidates.find((candidate) => {
        ...
        return (
          definition.success &&
          definition.data.siteObjectId === site.id &&
          definition.data.path === path
        );
      });
      const definition = publicPageSchema.safeParse(page?.data);
      if (!page || !definition.success) {
      ...
      const [company] = await db
        .select({ name: companies.name })
        .from(companies)
        .where(eq(companies.id, site.companyId))
        .limit(1);
      headers(res);
      res.json(
        pagePayload(
