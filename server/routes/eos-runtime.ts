        ...
        body: {
          blueprint: {
            ...
            nativeAssets: materializeNativeBusinessStarters({
              ...
              targetCustomer: access.company.targetCustomer || "",
              goals: access.company.goals || "",
              stage: access.company.stage || "",
              businessModel: businessModel || "",
              formation: formation.formation,
              founderVision: typeof founderProfile.vision === "string" ? founderProfile.vision : "",
              founderValues: typeof founderProfile.values === "string" ? founderProfile.values : "",
              decisionStyle: typeof founderProfile.decisionStyle === "string" ? founderProfile.decisionStyle : "",
              workingStyle: typeof founderProfile.workingStyle === "string" ? founderProfile.workingStyle : "",
              ...
              ownerRoleKeys: {
                ...
                executive: blueprint.roles.find((role) => role.key === "company_ceo")?.key,
      ...
      await db.transaction(async (tx) => {
        for (const mission of plan.setupMissions) {
          ...
          const packet = {
            ...
            classification: "internal",
            requiresApproval: false,
            toolPack: ["Organization Compiler", "Org Studio", "Executive Assistant", ...(mission.sourceAuthority === "declared_external_inventory" ? ["Systems"] : [])],
            evidenceRequirements: mission.completionEvidence,
            capabilityInstanceId: null,
            processDefinitionId: null,
            resourceIds: [],
            expectedOutput: `${mission.completionEvidence.join("; ")}. Native fallback: ${mission.nativeFallback}`,
            acceptanceCriteria: `The required inputs are explicit, the source boundary is respected (${mission.scopeBoundary}), and the named completion evidence is recorded for: ${mission.title}.`,
            constraintsPolicies: `Source authority: ${mission.sourceAuthority}. ${mission.scopeBoundary} This setup mission cannot activate a company, grant authority, connect a provider, or claim operational proof outside the governed route for that effect.`,
            failureEscalationCompensation: mission.failureRecoveryPath,
            humanFallback: mission.nativeFallback,
