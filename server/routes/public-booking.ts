...
export function registerPublicBookingRoutes(app: Express): void {
  ...
  app.post("/api/public/bookings/:calendarId/reservations", route(async (req, res) => {
    ...
    await db.transaction(async (tx) => {
      ...
      const common = { companyId: calendar.companyId, classification: "confidential", visibility: "organization", ownerSeatId: calendar.ownerSeatId, parentObjectId: calendar.id, evidenceIds: [], version: 1, recordedByUserId: calendar.recordedByUserId, createdAt: now, updatedAt: now, archivedAt: null, state: "active" } as const;
      ...
      const event = { ...common, id: bookingEventId, instrumentKey: "calendar", objectType: "event", objectKey: `public-booking-event:${calendar.id}:${bookingEventId}`, title: `Booking · ${input.name}`, summary: input.note || `Public booking for ${calendar.title}.`, data: { calendarObjectId: calendar.id, startsAt: startsAt.toISOString(), endsAt: endsAt.toISOString(), participantReferences: [input.email], bookingSource: "public_eos", relationshipObjectId: relationship.id }, sourceReference };
      const booking = { ...common, id: bookingId, instrumentKey: "calendar", objectType: "booking", objectKey: `public-booking:${calendar.id}:${bookingId}`, title: `Booking · ${input.name}`, summary: input.note || `Public booking for ${calendar.title}.`, data: { eventObjectId: event.id, bookedByReference: input.email, relationshipObjectId: relationship.id, consentVersion: definition.publicBookingConsentVersion, bookedAt: now.toISOString() }, sourceReference };
