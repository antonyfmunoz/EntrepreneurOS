...
async function stateSigningKey(): Promise<CryptoKey> { const material = Buffer.from(hkdfSync("sha256", stateSecret(), "entrepreneuros/gohighlevel-oauth-state/v1", "authorization-response-integrity", 32)); return webcrypto.subtle.importKey("raw", material, { name: "HMAC", hash: "SHA-256" }, false, ["sign", "verify"]); }
function stateMessage(payload: string) { return new TextEncoder().encode(`gohighlevel:${payload}`); }
async function signState(payload: string) { return Buffer.from(await webcrypto.subtle.sign("HMAC", await stateSigningKey(), stateMessage(payload))).toString("base64url"); }
