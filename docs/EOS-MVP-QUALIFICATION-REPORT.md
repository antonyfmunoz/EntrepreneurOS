...
| Unit and isolated-database integration | Pass locally | 25 files, 119 tests, no skips against disposable PostgreSQL; the prior protected merge `56da5dab64ece8254b96876584775225509de75d` passed 112 tests, and protected-branch qualification must repeat all 119 for this exact increment |
...
| Supply-chain evidence | Pass through prior protected merge | Protected-branch qualification generates a production-only CycloneDX SBOM, hashes and retains it, and issues SLSA provenance on push; merge `56da5dab64ece8254b96876584775225509de75d` passed qualification run `31746400437` and must be superseded by exact-merge evidence for this increment |
| Static application security | Pass through prior protected merge | CodeQL run `31746400637` passed with zero open alerts. The only accepted raw SARIF exceptions are the exact Gmail and Notion OAuth-state HMAC verification rule/path pairs, capped at two; both authenticate short-lived, user-bound state with timing-safe comparison and are not password storage. This increment still requires exact-merge CodeQL repetition |
