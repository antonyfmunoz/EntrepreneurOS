...
- `EOS_MALWARE_SCAN_MODE=clamav`, with `EOS_CLAMAV_HOST=127.0.0.1` and `EOS_CLAMAV_PORT=3310`. The production image owns the engine, current signature database, and continuous signature updates; it does not expose the daemon publicly or require a scanner SaaS credential.
...
`GET /api/ready` returns 200 only when the database is reachable, hosted required configuration is present, and the EOS-owned ClamAV daemon answers its loopback health probe. `GET /api/health` is a liveness check and does not establish release readiness.
