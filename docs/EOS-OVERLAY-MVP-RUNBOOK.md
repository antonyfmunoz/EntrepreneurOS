...
- permanently quarantined legacy global task, workflow, conversation, action, CRM, folder, document, analytics, agent, assistant, and provider-control routes; the generic task/workflow/conversation/agent/assistant handler modules and unreachable legacy UI are removed from the runtime entirely, while stable `410 Gone` tombstones direct old clients to their company-scoped EOS replacements;
- Google Workspace and Notion connect, verify, and disconnect controls resolve the selected company and active seat, require Systems visibility, record an EOS policy decision, and force the signed OAuth return path to that company; callback credentials remain encrypted and tied to the authorizing human;
...
- `EOS_UNTRUSTED_UPLOADS_ENABLED=false` runs the current single-operator deployment in trusted-source mode. Direct candidate files, arbitrary PDF registration/revision, drawn signatures, and uploaded signature images remain blocked; HTTPS references, structured submissions, EOS-generated agreements, and typed signatures remain available. To enable direct uploads later, set this to `true` and configure the preserved ClamAV scanner (`EOS_MALWARE_SCAN_MODE=clamav`, loopback host, port `3310`) before release readiness can pass.
...
`GET /api/ready` returns 200 only when the database is reachable and hosted required configuration is present. File ingress must be explicitly declared: trusted-source mode closes new binary uploads; scanner-backed mode requires a configured scanner and, for ClamAV, a successful loopback health probe. Unknown or incomplete configuration fails closed. `GET /api/runtime-capabilities` reports the enforced ingress mode and available signature methods without secrets. `GET /api/health` is a liveness check and does not establish release readiness.

Trusted-source mode keeps the Fly runtime at 512 MB and does not launch ClamAV. It does not certify historical files as clean or turn arbitrary Drive links into safe-file attestations. Existing records remain under their existing access and custody controls. Do not automatically download referenced binary content; use structured information, native generated agreements, and typed signatures. Direct candidate/voice files, new arbitrary PDFs, PDF revisions, and image captures require scanner-backed mode. Before enabling ClamAV later, qualify its resource allocation, current signatures, clean/infected/failure outcomes, and protected release evidence.
...
Candidate file uploads accept PDF, PNG, JPEG, UTF-8 text, and bounded audio up to 10 MB. EOS stores files with private permissions outside PostgreSQL, records SHA-256 and metadata, and exposes no download until the EOS-owned scanner returns `clean`. Missing or failed scanning keeps the object quarantined; infected objects are removed. Uploaded native e-sign PDFs, replacement revisions, and image-based signature captures must also receive a clean verdict before storage or parsing continues. The local filesystem adapter is suitable for standalone development or a deliberately managed persistent volume; public multi-instance promotion still requires a shared encrypted object-store adapter or an explicitly accepted single-writer volume design, plus scanner, backup/restore, retention, and deletion evidence.
