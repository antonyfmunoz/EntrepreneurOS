...
export default function CompanySetupPage() {
  ...
  const completeJourney = () => {
    ...
    const founderProfile = { vision: founderVision.trim(), values: founderValues.trim(), decisionStyle: decisionStyle.trim(), workingStyle: workingStyle.trim(), industry: industry.trim(), businessModel, operatingFormation: formation, teamSnapshot: teamSnapshot.trim(), currentReality: { ...currentReality, assetsAndObligations: currentReality.assetsAndObligations.trim(), marketAndDemand: currentReality.marketAndDemand.trim(), economicsAndCapital: currentReality.economicsAndCapital.trim(), bottleneckAndGovernance: currentReality.bottleneckAndGovernance.trim(), evidenceGaps: currentReality.evidenceGaps.trim() }, existingSystems: existingSystems.split(/[\n,]/).map((item) => item.trim()).filter(Boolean), ...(formation !== "agent_first" ? { teamRosterPlan: { version: "team-roster-plan-v1", entries: rosterEntries } } : {}), setupJourneyVersion: "company-mission-journey-v1" };
    saveCompany.mutate({
