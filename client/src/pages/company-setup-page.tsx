...
type CurrentRealityDraft = {
  assetsAndObligations: string;
  marketAndDemand: string;
  economicsAndCapital: string;
  bottleneckAndGovernance: string;
  evidenceGaps: string;
  evidenceConfidence: "high" | "medium" | "low" | "unknown";
};

const blankCurrentReality: CurrentRealityDraft = {
  assetsAndObligations: "",
  marketAndDemand: "",
  economicsAndCapital: "",
...
function stagedCurrentReality(profile: Record<string, unknown> | undefined): CurrentRealityDraft | null {
  const raw = profile?.currentReality;
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
  const value = raw as Record<string, unknown>;
  const confidence = value.evidenceConfidence;
  return {
    assetsAndObligations: typeof value.assetsAndObligations === "string" ? value.assetsAndObligations : "",
    marketAndDemand: typeof value.marketAndDemand === "string" ? value.marketAndDemand : "",
    economicsAndCapital: typeof value.economicsAndCapital === "string" ? value.economicsAndCapital : "",
    bottleneckAndGovernance: typeof value.bottleneckAndGovernance === "string" ? value.bottleneckAndGovernance : "",
    evidenceGaps: typeof value.evidenceGaps === "string" ? value.evidenceGaps : "",
    evidenceConfidence: confidence === "high" || confidence === "medium" || confidence === "low" || confidence === "unknown"
      ? confidence
      : "unknown",
  };
}
...
export default function CompanySetupPage() {
  ...
  const [primaryGrowthMotion, setPrimaryGrowthMotion] = useState("");
  ...
  const [operatingCadence, setOperatingCadence] = useState<CompanyMissionInput["operatingCadence"]>("weekly");
  ...
  const [currentReality, setCurrentReality] = useState<CurrentRealityDraft>(blankCurrentReality);
  const [currentRealityConfirmed, setCurrentRealityConfirmed] = useState(false);
  ...
  useEffect(() => {
    ...
    setPrimaryGrowthMotion(profileString(profile, "primaryGrowthMotion"));
    ...
    const savedCadence = profileString(profile, "operatingCadence");
    if (savedCadence === "weekly" || savedCadence === "biweekly" || savedCadence === "monthly") setOperatingCadence(savedCadence);
    ...
    const savedCurrentReality = stagedCurrentReality(profile);
    if (savedCurrentReality) {
      setCurrentReality(savedCurrentReality);
      setCurrentRealityConfirmed(true);
    }
  ...
  const missionInput: CompanyMissionInput = {
    ...
    stage, industry, businessModel, offer, targetCustomer, primaryGrowthMotion, assistantName, founderVision, founderValues, decisionStyle, workingStyle, operatingCadence, goals, formation, teamSnapshot,
    existingSystems: existingSystems.split(/[\n,]/).map((item) => item.trim()).filter(Boolean),
    currentRealityProvided: currentRealityConfirmed,
  };
  ...
  const advance = () => {
    ...
    if (currentStep === 6) setCurrentRealityConfirmed(true);
    setCurrentStep((step) => Math.min(step + 1, companyMissionJourney.length - 1));
  };
  const completeJourney = () => {
    ...
    const founderProfile = { vision: founderVision.trim(), values: founderValues.trim(), decisionStyle: decisionStyle.trim(), workingStyle: workingStyle.trim(), operatingCadence, industry: industry.trim(), businessModel, primaryGrowthMotion: primaryGrowthMotion.trim(), operatingFormation: formation, teamSnapshot: teamSnapshot.trim(), currentReality: { ...currentReality, assetsAndObligations: currentReality.assetsAndObligations.trim(), marketAndDemand: currentReality.marketAndDemand.trim(), economicsAndCapital: currentReality.economicsAndCapital.trim(), bottleneckAndGovernance: currentReality.bottleneckAndGovernance.trim(), evidenceGaps: currentReality.evidenceGaps.trim() }, existingSystems: existingSystems.split(/[\n,]/).map((item) => item.trim()).filter(Boolean), ...(formation !== "agent_first" ? { teamRosterPlan: { version: "team-roster-plan-v1", entries: rosterEntries } } : {}), setupJourneyVersion: "company-mission-journey-v1" };
    saveCompany.mutate({
      ...
      <section className="mx-auto grid w-full max-w-6xl gap-8 pb-10 lg:grid-cols-[260px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 rounded-2xl border bg-white p-5 shadow-[0_12px_40px_rgba(74,42,143,0.08)] sm:p-8">
          ...
          <div className="min-h-[360px] py-7">
            ...
            {currentStep === 6 && <div className="max-w-3xl space-y-5"><div className="rounded-xl border border-primary/20 bg-primary/5 p-5"><Sparkles className="mb-3 h-6 w-6 text-primary"/><h3 className="font-semibold">Start from what is true today.</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">This is the same company model for a new founder and an established business. Fill in what you know; leave what you do not know visible. EOS will compile unknowns into governed discovery work instead of pretending they are facts.</p></div><Field label="Assets and obligations" hint="What already exists or must be honored: customers, active work, contracts, intellectual property, key assets, vendor commitments, deadlines, or material liabilities."><Textarea value={currentReality.assetsAndObligations} onChange={(event) => setCurrentReality((current) => ({ ...current, assetsAndObligations: event.target.value }))} placeholder="Example: Two active clients, a signed delivery agreement, a small content library, and a monthly contractor commitment." className="min-h-28" /></Field><Field label="Market and demand context" hint="What is known about the buyer, geography, demand access, competition, buying motion, or applicable market constraints."><Textarea value={currentReality.marketAndDemand} onChange={(event) => setCurrentReality((current) => ({ ...current, marketAndDemand: event.target.value }))} placeholder="Example: Founder-led B2B outreach to U.S. creative-service firms; referrals and targeted outbound are the first demand paths." className="min-h-28" /></Field><Field label="Economics and capital constraints" hint="Record the pricing model, revenue mechanics, meaningful cost or margin constraints, cash timing, and capital limits that should shape early decisions."><Textarea value={currentReality.economicsAndCapital} onChange={(event) => setCurrentReality((current) => ({ ...current, economicsAndCapital: event.target.value }))} placeholder="Example: Fixed-fee offer, contractor-heavy delivery, cash collected at kickoff, no outside capital planned." className="min-h-28" /></Field><Field label="Operating bottleneck and governance constraints" hint="Name the most important current constraint, capacity issue, policy, compliance obligation, decision-right limit, or risk that EOS must respect."><Textarea value={currentReality.bottleneckAndGovernance} onChange={(event) => setCurrentReality((current) => ({ ...current, bottleneckAndGovernance: event.target.value }))} placeholder="Example: Founder capacity is the bottleneck; contracts and outbound claims require founder review." className="min-h-28" /></Field><div className="grid gap-5 sm:grid-cols-2"><Field label="Evidence confidence" hint="How confident are you that this baseline is current and source-backed?"><RadioGroup value={currentReality.evidenceConfidence} onValueChange={(value) => setCurrentReality((current) => ({ ...current, evidenceConfidence: value as CurrentRealityDraft["evidenceConfidence"] }))} className="grid grid-cols-2 gap-2">{(["high", "medium", "low", "unknown"] as const).map((value) => <Choice key={value} value={value} label={value === "unknown" ? "Not assessed" : `${value[0].toUpperCase()}${value.slice(1)}`} selected={currentReality.evidenceConfidence === value} />)}</RadioGroup></Field><Field label="Evidence gaps or contradictions" hint="Optional. Name missing sources, conflicting facts, or questions EOS must resolve before relying on this baseline."><Textarea value={currentReality.evidenceGaps} onChange={(event) => setCurrentReality((current) => ({ ...current, evidenceGaps: event.target.value }))} placeholder="Example: Need to verify active contract terms and current delivery capacity." /></Field></div><p className="rounded-xl bg-muted/50 p-4 text-sm text-muted-foreground">Continuing records this as the current baseline. Any missing or low-confidence parts become an explicit discovery mission with conservative decision boundaries—not hidden defaults.</p></div>}
            {currentStep === 7 && <div className="max-w-3xl space-y-5"><div className="rounded-xl border border-primary/20 bg-primary/5 p-5"><Network className="mb-3 h-6 w-6 text-primary"/><h3 className="font-semibold">Native first. Integrations when useful.</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">EOS will create the company’s native operating foundation now. Connect a CRM, accounting system, document service, or inbox later only to reconcile existing data or use a specialist external rail. No integration is required to use the company graph, roles, work, documents, or native instruments.</p></div><Field label="Existing systems or record sources" hint="Optional. Name tools or data sources that already hold company records, one per line. This creates an inventory for a later governed reconciliation plan; it does not connect, import, or grant any provider access."><Textarea value={existingSystems} onChange={(event) => setExistingSystems(event.target.value)} placeholder="QuickBooks\nGoogle Workspace\nLegacy CRM" className="min-h-28" /></Field><div className="rounded-xl bg-muted/50 p-5"><p className="font-medium">What happens next</p><ul className="mt-3 space-y-2 text-sm text-muted-foreground"><li>• Compile the native organization, manifest, and governed setup work from this one company model.</li><li>• Open Org Studio and shape the real reporting graph, role agents, responsibilities, and tools.</li><li>• Use Systems only when an external provider adds value or holds historical data.</li></ul></div></div>}
          </div>
