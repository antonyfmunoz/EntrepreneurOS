...
type BillingStatus = {
  mode: "internal_operator" | "public_paid_saas";
  configured: boolean;
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-9">
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as SettingsTab)}>
          <TabsList className="mb-6 grid h-auto w-full grid-cols-2 justify-start gap-1 rounded-2xl bg-muted p-1.5 sm:inline-flex sm:w-auto">
            ...
            <TabsTrigger className={tabClass("billing")} value="billing">Access & payments</TabsTrigger>
          ...
          <TabsContent value="profile">
            <SettingsCard>
              ...
              {userProfile.isLoading ? <div className="h-32 animate-pulse rounded-xl bg-muted" /> : <div className="space-y-5">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-5"><button type="button" onClick={() => document.querySelector<HTMLButtonElement>('[aria-label="Notifications"]')?.click()} className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:underline"><Bell className="h-4 w-4" />Open in-app notifications</button><Button onClick={saveProfile} disabled={updateProfile.isPending}>{updateProfile.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Check className="mr-2 h-4 w-4" />}Save profile</Button></div>
          ...
          <TabsContent value="billing">
            <SettingsCard>
              <div className="mb-6 flex items-start gap-3"><CreditCard className="mt-0.5 h-5 w-5 text-primary" /><div><h2 className="text-xl font-semibold">Access & payments</h2><p className="mt-1 text-sm text-muted-foreground">Your EOS access and the boundary between platform access and company revenue.</p></div></div>
