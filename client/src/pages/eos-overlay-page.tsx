...
export default function EosOverlayPage() {
  ...
  const membershipMutation = useMutation({
    mutationFn: () => requestJson<JsonRecord>("POST", `${root}/invitations`, {
    ...
    onSuccess: async () => {
      setMembershipEmail("");
      await refresh();
      toast({ title: "Invitation sent", description: "The seat remains unoccupied until the recipient signs in and accepts the role." });
    },
    onError: (error) => showMutationError("Invitation", error),
  });

  const revokeInvitationMutation = useMutation({
    mutationFn: (invitationId: string) => requestJson<JsonRecord>("POST", `${root}/invitations/${invitationId}/revoke`),
    onSuccess: async () => {
      await refresh();
      toast({ title: "Invitation revoked", description: "The invitation can no longer grant access." });
    },
      ...
      <div className="space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="organization" className="space-y-4">
            ...
            {(["founder", "company_ceo"].includes(principalContext?.role)) && <Card><CardHeader><CardTitle>Build the operating hierarchy</CardTitle><CardDescription>Create an accountable seat, position it under a supervisor, then invite a person to accept it. Once accepted, the existing Role Agent becomes that person's assistant.</CardDescription></CardHeader><CardContent className="grid gap-6 lg:grid-cols-2">
              <div className="space-y-3"><p className="eos-label">Create seat</p><Input value={seatTitle} onChange={(event) => setSeatTitle(event.target.value)} placeholder="Seat title, e.g. Head of Growth" /><Input value={seatAgentName} onChange={(event) => setSeatAgentName(event.target.value)} placeholder="Role Agent name" /><select aria-label="Seat role" value={seatKind} onChange={(event) => setSeatKind(event.target.value)} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="company_ceo">Company CEO</option><option value="functional_executive">Functional executive</option><option value="manager">Manager</option><option value="individual_contributor">Individual contributor</option><option value="external">External collaborator</option></select><select aria-label="Reporting supervisor" value={seatSupervisorId} onChange={(event) => setSeatSupervisorId(event.target.value)} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">Report to my active seat</option>{(organizationQuery.data?.seats || []).map((seat: JsonRecord) => <option key={seat.id} value={seat.id}>{seat.title}</option>)}</select><Button disabled={seatTitle.trim().length < 2 || seatAgentName.trim().length < 2 || seatMutation.isPending} onClick={() => seatMutation.mutate()}><Plus className="mr-2 h-4 w-4" />{seatMutation.isPending ? "Creating…" : "Create accountable seat"}</Button></div>
              <div className="space-y-3"><p className="eos-label">Invite person to seat</p><Input type="email" value={membershipEmail} onChange={(event) => setMembershipEmail(event.target.value)} placeholder="Work email address" /><select aria-label="Seat for invitation" value={membershipSeatId} onChange={(event) => setMembershipSeatId(event.target.value)} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">Choose an unoccupied seat</option>{(organizationQuery.data?.seats || []).filter((seat: JsonRecord) => !seat.occupantUserId && !(organizationQuery.data?.invitations || []).some((invitation: JsonRecord) => invitation.seatId === seat.id && ["pending", "pending_delivery"].includes(invitation.status))).map((seat: JsonRecord) => <option key={seat.id} value={seat.id}>{seat.title}</option>)}</select><Button variant="secondary" disabled={!membershipEmail.includes("@") || !membershipSeatId || membershipMutation.isPending} onClick={() => membershipMutation.mutate()}>{membershipMutation.isPending ? "Sending…" : "Send secure invitation"}</Button><p className="text-xs text-muted-foreground">Access begins only after the recipient signs in with this verified email and accepts the exact role. Active assignments: {organizationQuery.data?.memberships?.length || 0}.</p>{(organizationQuery.data?.invitations || []).filter((invitation: JsonRecord) => ["pending", "pending_delivery"].includes(invitation.status)).map((invitation: JsonRecord) => { const seat = (organizationQuery.data?.seats || []).find((item: JsonRecord) => item.id === invitation.seatId); return <div key={invitation.id} className="flex items-center justify-between gap-3 rounded-lg border p-3 text-sm"><div className="min-w-0"><p className="truncate font-medium">{invitation.email}</p><p className="truncate text-xs text-muted-foreground">{seat?.title || "Organizational seat"} · {invitation.status === "pending_delivery" ? "delivery in progress" : `expires ${new Date(invitation.expiresAt).toLocaleDateString()}`}</p></div><Button size="icon" variant="ghost" aria-label={`Revoke invitation for ${invitation.email}`} disabled={revokeInvitationMutation.isPending} onClick={() => revokeInvitationMutation.mutate(invitation.id)}><X className="h-4 w-4" /></Button></div>; })}</div>
            </CardContent></Card>}
          </TabsContent>
