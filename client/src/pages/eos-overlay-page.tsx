                ...
                {agreementConfig.eSignProvider === "eos_native" ? <>
                  <label className="space-y-2 text-sm font-medium lg:col-span-2">Counsel-linked EOS document version
                    <select className="h-10 w-full rounded-md border border-input bg-background px-3" value={agreementConfig.eSignTemplateReference || ""} onChange={(event) => { const document = nativeDocumentVersions.find((item) => item.id === event.target.value); setAgreementConfig((value) => ({ ...value, eSignTemplateReference: event.target.value, agreementVersion: document?.documentVersion || value.agreementVersion })); }}>
                      <option value="">Select immutable PDF version</option>
                      {nativeDocumentVersions.map((item) => <option key={item.id} value={item.id}>{item.title} — {Number(item.pageCount || 1)} page{Number(item.pageCount || 1) === 1 ? "" : "s"} — {item.documentVersion} — {String(item.sourceSha256).slice(0, 10)}…</option>)}
                    </select>
                  </label>
                  {untrustedUploadsEnabled ? <div className="space-y-3 rounded-lg border bg-background p-3 lg:col-span-2">
                    <div><p className="text-sm font-medium">Register a new counsel-approved PDF</p><p className="text-xs text-muted-foreground">Uses the recorded effective version, exact-language reference, and verified counsel Evidence. Place signer fields before the source PDF becomes immutable.</p></div>
                    <Input type="file" accept="application/pdf,.pdf" onChange={(event) => { setNativeDocumentFile(event.target.files?.[0] || null); setNativeDocumentFields([]); }}/>
...
export default function EosOverlayPage() {
  ...
  const slackProviderConnectionsQuery = useQuery<JsonRecord>({
    queryKey: [root, roleScopeKey, "slack-provider-connections"],
    queryFn: () => requestJson("GET", `${root}/integrations/slack/connections`),
    enabled: Boolean(
      companyId &&
      (contextQuery.data?.principalContext?.allowedSurfaces || []).includes("systems"),
    ),
  ...
  const companyVaultBindingMutation = useMutation({
    mutationFn: async ({
      integration,
      draft,
    }: {
      ...
      const providerDefaults = providerKey === "stripe"
        ? {
            adapterKind: "api_key",
            adapterReference: "eos-stripe-commercial-adapter",
            transport: "HTTPS + Stripe API + signed webhook",
            nativePermissions: ["merchant.read", "payment_links.write", "webhook.verify"],
            operations: ["merchant.identity.read", "payment_link.create", "payment.receipt.reconcile"],
            expectedEvents: ["checkout.session.completed", "payment_intent.succeeded", "invoice.paid"],
      ...
      if (existing?.id) {
        return requestJson<JsonRecord>(
          "PATCH",
          `${root}/integration-bindings/${existing.id}`,
          {
            ...common,
            ...(draft.administratorReference.trim()
      ...
      return requestJson<JsonRecord>("POST", `${root}/integration-bindings`, {
        name: `${integration.name} company connection`,
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <CardContent className="space-y-5">
        ...
        {(integration.id === "google_workspace" || integration.id === "notion" || integration.id === "quickbooks" || integration.id === "slack" || integration.id === "gohighlevel" || integration.id === "docusign" || integration.id === "stripe") && (
          <div className="rounded-xl border border-border/70 bg-muted/20 p-4">
