...
export function NativeProjectsStudio({ root, roleScopeKey, activeSeatId, seats, processes, canUseWorkflows, canExecute, canDecide, onOpenOperations }: {
  root: string; roleScopeKey: string; activeSeatId: string; seats: Seat[]; processes: Json[]; canUseWorkflows: boolean; canExecute: boolean; canDecide: boolean; onOpenOperations?: () => void;
}) {
  ...
  const workflowRunsQuery = useQuery<Json>({
    queryKey: [root, roleScopeKey, "native-projects-client-onboarding-runs"],
    enabled: canUseWorkflows,
    retry: false,
    queryFn: async () => (await apiRequest("GET", `${root}/workflow-runtime`)).json(),
  });
  ...
  // A compiled company blueprint identifies this process by workflowKey. It
  // becomes callable only after the same qualification and release gate used
  // by a no-code workflow authored in Operations.
  const releasedClientOnboardingProcess = useMemo(() => processes
    .filter((process) => process.workflowKey === "client-onboarding" && process.releaseState === "released" && ["implemented", "pre_live_qualified", "field_qualified"].includes(process.qualificationState))
    .sort((left, right) => Number(right.version || 0) - Number(left.version || 0))[0] || null, [processes]);
  const visibleClientOnboardingProcess = useMemo(() => processes
    .filter((process) => process.workflowKey === "client-onboarding")
    .sort((left, right) => Number(right.version || 0) - Number(left.version || 0))[0] || null, [processes]);
  const selectedProjectOnboardingRuns = useMemo(() => ((workflowRunsQuery.data?.runs || []) as Json[])
    .filter((run) => run.input?.deliveryProjectObjectId === selectedProject?.id), [selectedProject?.id, workflowRunsQuery.data]);
  const activeProjectOnboardingRun = selectedProjectOnboardingRuns.find((run) => !["failed", "cancelled"].includes(String(run.state))) || null;
  useEffect(() => {
  ...
  const refresh = async () => Promise.all([
    queryClient.invalidateQueries({ queryKey: [root, roleScopeKey, "native-projects"] }),
    queryClient.invalidateQueries({ queryKey: [root, roleScopeKey, "native-tasks"] }),
    queryClient.invalidateQueries({ queryKey: [root, roleScopeKey, "native-projects-client-onboarding-runs"] }),
  ]);
  ...
  const startClientOnboarding = useMutation({
    mutationFn: async () => {
      if (!selectedProject || !selectedProjectOrderId || !releasedClientOnboardingProcess)
        throw new Error("A released client-onboarding process and delivery project are required before onboarding can start.");
      return (await apiRequest("POST", `${root}/workflow-runs`, {
        processDefinitionId: releasedClientOnboardingProcess.id,
        executionMode: "manual",
        idempotencyKey: commandKey("native-delivery-client-onboarding"),
        input: {
          source: "native_delivery_project",
          deliveryProjectObjectId: selectedProject.id,
          orderObjectId: selectedProjectOrderId,
          sourceOpportunityObjectId: selectedProjectOpportunityId || undefined,
          externalEffectsPermitted: false,
        },
        classification: selectedProject.classification === "restricted" ? "restricted" : "confidential",
      })).json();
    },
  ...
  return <Card data-testid="native-projects-studio">
    ...
    <CardContent className="space-y-5">
      ...
      {selectedProject && <section className="rounded-xl border border-primary/25 bg-primary/[0.03] p-4"><p className="eos-label">Configure selected project</p>{selectedProjectOrderId && <><div className="mt-3 rounded-lg border bg-background/70 p-3"><p className="text-sm font-medium">Commercial source</p>{canViewCommerce ? <><p className="mt-1 text-sm text-muted-foreground">{selectedProjectOrder ? `This delivery project was created from the native order “${selectedProjectOrder.title}”.` : "The linked native order is no longer visible to this role."}</p>{selectedProjectOrder && <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground"><span>Buyer: {String(selectedProjectOrder.data?.buyerReference || "Governed buyer")}</span>{selectedProjectOpportunity && <span>CRM opportunity: {selectedProjectOpportunity.title}</span>}</div>}</> : <p className="mt-1 text-sm text-muted-foreground">This project has a commercial origin, but your role has not been granted Commerce. EOS keeps that order and buyer data private while you operate the assigned work.</p>}</div><div className="mt-3 rounded-lg border bg-background/70 p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium">Client onboarding</p><p className="mt-1 text-xs text-muted-foreground">Start the company&apos;s released client-onboarding workflow with this delivery project and native order as bounded context. Starting a run records work only; it does not collect payment, send a message, grant access, or perform a provider action.</p></div>{activeProjectOnboardingRun && <Badge variant={stateVariant(String(activeProjectOnboardingRun.state))}>{String(activeProjectOnboardingRun.state).replaceAll("_", " ")}</Badge>}</div>{canUseWorkflows ? <div className="mt-3">{activeProjectOnboardingRun ? <div className="flex flex-wrap items-center gap-2"><p className="text-xs text-muted-foreground">EOS already has a client-onboarding run for this delivery project. Continue its governed steps in Operations instead of making a duplicate checklist.</p>{onOpenOperations && <Button size="sm" variant="outline" onClick={onOpenOperations}>Open governed onboarding run</Button>}</div> : releasedClientOnboardingProcess ? <><p className="mb-2 text-xs text-muted-foreground">Bound process: {releasedClientOnboardingProcess.name} · v{releasedClientOnboardingProcess.version}</p><Button size="sm" disabled={!canExecute || startClientOnboarding.isPending} onClick={() => startClientOnboarding.mutate()}>{startClientOnboarding.isPending ? "Starting onboarding…" : "Start client onboarding"}</Button></> : <p className="text-xs text-muted-foreground">{visibleClientOnboardingProcess ? `The compiled client-onboarding process is ${visibleClientOnboardingProcess.releaseState} / ${visibleClientOnboardingProcess.qualificationState}. Complete its existing qualification and release gate in Operations before using it for live delivery.` : "No client-onboarding process is visible to this role. A founder or workflow owner can compile or grant access to the company process in Operations."}</p>}</div> : <p className="mt-3 text-xs text-muted-foreground">Your role can operate this project but has not been assigned Workflows, so EOS does not expose or start an onboarding process from this workspace.</p>}</div></>}<div className="mt-3 grid gap-3 md:grid-cols-2"><div><Label htmlFor="native-edit-project-title">Project name</Label><Input id="native-edit-project-title" className="mt-1" value={editingProjectTitle} onChange={(event) => setEditingProjectTitle(event.target.value)} /></div><div><Label htmlFor="native-edit-project-owner">Accountable owner</Label><select id="native-edit-project-owner" className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm" value={editingProjectOwnerId} onChange={(event) => setEditingProjectOwnerId(event.target.value)}>{activeSeats.map((seat) => <option key={seat.id} value={seat.id}>{seat.title}</option>)}</select></div><div className="md:col-span-2"><Label htmlFor="native-edit-project-objective">Business outcome</Label><Textarea id="native-edit-project-objective" className="mt-1" value={editingProjectObjective} onChange={(event) => setEditingProjectObjective(event.target.value)} /></div><div className="md:col-span-2"><Button size="sm" disabled={!canExecute || editingProjectTitle.trim().length < 2 || editingProjectObjective.trim().length < 3 || !editingProjectOwnerId || saveProject.isPending} onClick={() => saveProject.mutate()}>{saveProject.isPending ? "Saving…" : "Save native project"}</Button></div></div></section>}
