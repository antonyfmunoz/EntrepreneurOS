...
export function NativeWorkflowComposer({
  ...
  processes: Json[];
  /**
   * Values from the company mission, passed through the same materializer as
   * the Company Mission compiler.  This keeps a no-code workflow derived from
   * a starter specific to the active company instead of leaving generic
   * template tokens for an operator to discover and replace manually.
   */
  companyContext: NativeWorkflowStarterVariables;
  canExecute: boolean;
  canDecide: boolean;
  ...
  const applyStarter = (starter: NativeWorkflowStarter) => {
    const materialized = materializeNativeWorkflowStarter(starter.key, companyContext);
    setEditingId(""); setName(materialized.name); setPurpose(materialized.purpose); setOutcome(materialized.outcome); setTrigger(materialized.trigger); setApprovalGates(materialized.approvals.join("\n")); setBranchConditions(materialized.branches.join("\n")); setTemplateAncestry(`native_workflow_starter.${starter.key}.v1`); setSteps(materialized.steps.map((step) => ({ ...step, id: `step-${crypto.randomUUID().slice(0, 8)}` }))); setError("");
  };
