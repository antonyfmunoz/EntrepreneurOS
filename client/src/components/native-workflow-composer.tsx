...
const nativeTools = ["operations", "tasks", "crm", "messages", "calendar", "docs", "sheets", "websites", "finance", "conference_rooms"];
...
export function NativeWorkflowComposer({
  ...
  const clear = () => {
    setEditingId(""); setCapabilityId(""); setName(""); setPurpose(""); setOutcome(""); setTrigger(""); setApprovalGates(""); setBranchConditions(""); setSteps([newStep()]); setError("");
  };
  ...
  const payload = () => ({
    ...
    templateAncestry: "native_workflow_composer.v1", applicableOverlays: [], supportingActorKeys: [], requiredAuthority: Array.from(new Set(steps.map((step) => step.authorityClass))), disclosureScope: "internal",
