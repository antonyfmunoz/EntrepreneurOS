...
type MessageDeliveryState = "native_recorded" | "provider_intent" | "provider_delivery_planned" | "manual_external_intent";
...
export function NativeMessageHub({ root, roleScopeKey, activeSeatId, seats, canExecute, canDecide }: {
  ...
  const [editingConversationPurpose, setEditingConversationPurpose] = useState("");
  ...
  useEffect(() => {
    setEditingConversationTitle(String(selectedConversation?.title || ""));
    setEditingConversationPurpose(String(selectedConversation?.summary || ""));
  }, [selectedConversation?.id, selectedConversation?.version]);
  ...
  // Email and Slack reuse their structured delivery fields for either a
  // manual record or a governed provider plan. Other external channels need
  // a compact native evidence/reference form of their own.
  const isManualExternalIntent = channel !== "native_eos" && !mayPlanProviderDelivery;
  ...
  const saveConversation = useMutation({
    mutationFn: async () => {
      if (!selectedConversation) throw new Error("Select a conversation to configure.");
      return (await apiRequest("PATCH", `${root}/instrument-objects/${selectedConversation.id}`, {
        expectedVersion: selectedConversation.version,
        title: editingConversationTitle.trim(),
        summary: editingConversationPurpose.trim(),
        data: { ...selectedConversation.data, operatingMode: "native_eos" },
        idempotencyKey: commandKey("message-conversation-configure"),
      })).json();
    },
    onSuccess: async (result) => { setSelectedConversationId(result.object.id); await refresh(); },
    onError: (cause: Error) => setError(cause.message),
  });
  ...
  const createMessageRecord = async (input: { deliveryState: MessageDeliveryState }) => {
  ...
  const createMessage = useMutation({
    // A direct external-channel record is intentionally not a provider action.
    // Only the governed-run path below may claim a provider delivery plan.
    mutationFn: async () => createMessageRecord({ deliveryState: channel === "native_eos" ? "native_recorded" : "manual_external_intent" }),
    ...
    <CardContent className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <section className="rounded-xl border p-4">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Native EOS messages are stored in this conversation{selectedThread ? ` under the ${selectedThread.title} thread` : ""}. Email and Slack can create a provider delivery plan only when a qualified company binding is selected. SMS, iMessage, Instagram, and other social entries are manual external intents until EOS has a governed provider contract for that channel.</p>
          <div className="mt-4 grid gap-3">
            ...
            {isManualExternalIntent && <div className="grid gap-3 rounded-lg border bg-muted/20 p-3">
              <div><Label htmlFor="message-manual-external-destination">{manualDestinationLabel(channel)} (optional)</Label><Input id="message-manual-external-destination" className="mt-1" value={deliveryDestination} onChange={(event) => setDeliveryDestination(event.target.value)} placeholder={channel === "instagram" ? "@clienthandle" : channel === "social" ? "Channel, handle, or thread" : "Recipient or contact reference"} /></div>
              <div><Label htmlFor="message-manual-external-reference">External thread or evidence reference (optional)</Label><Input id="message-manual-external-reference" className="mt-1" value={deliveryThreadReference} onChange={(event) => setDeliveryThreadReference(event.target.value)} placeholder="Conversation, message, or receipt reference" /></div>
              <p className="text-xs text-muted-foreground">EOS records this as a manual external intent only. It does not send through Apple, Meta, an SMS carrier, or any other provider. After the outside action is completed, record its evidence or receipt through the governed workflow.</p>
            </div>}
