...
export function NativeSheetsStudio({ root, roleScopeKey, canExecute, canDecide }: {
  ...
  const [editingWorkbookTitle, setEditingWorkbookTitle] = useState("");
  ...
  const selectedChart = charts.find((item) => item.id === selectedChartId) || null;
  ...
  useEffect(() => {
    setEditingWorkbookTitle(String(selectedWorkbook?.title || ""));
  }, [selectedWorkbook?.id, selectedWorkbook?.version]);
  ...
  useEffect(() => {
    ...
    setChartTitle(String(selectedChart.title || ""));
    setChartColumn(String(selectedChart.data?.dataRange || ""));
  }, [selectedChart?.id, selectedChart?.version]);
  ...
  const saveWorkbook = useMutation({
    mutationFn: async () => {
      if (!selectedWorkbook) throw new Error("Select a workbook to configure.");
      return (await apiRequest("PATCH", `${root}/instrument-objects/${selectedWorkbook.id}`, {
        expectedVersion: selectedWorkbook.version,
        title: editingWorkbookTitle.trim(),
        data: { ...selectedWorkbook.data, operatingMode: "native_eos" },
        idempotencyKey: commandKey("sheets-workbook-configure"),
      })).json();
    },
    onSuccess: async (result) => { setSelectedWorkbookId(result.object.id); await refresh(); },
    onError: (cause: Error) => setError(cause.message),
  });
  ...
  const createChart = useMutation({
    ...
    onSuccess: async (result) => { setSelectedChartId(result.object.id); await refresh(); },
    onError: (cause: Error) => setError(cause.message),
  });
  const saveChart = useMutation({
    mutationFn: async () => {
      if (!selectedChart) throw new Error("Select a chart view to configure.");
      const column = chartColumn.trim();
      if (!sheetColumns.includes(column)) throw new Error("Choose a worksheet column for the chart.");
      return (await apiRequest("PATCH", `${root}/instrument-objects/${selectedChart.id}`, {
        expectedVersion: selectedChart.version,
        title: chartTitle.trim(),
        summary: `Bar chart of ${column} from ${selectedSheet?.title || "this worksheet"}.`,
        data: { ...selectedChart.data, worksheetObjectId: selectedSheet?.id, chartType: "bar", dataRange: column, operatingMode: "native_eos" },
        idempotencyKey: commandKey("sheets-chart-configure"),
      })).json();
    },
    onSuccess: async (result) => { setSelectedChartId(result.object.id); await refresh(); },
    onError: (cause: Error) => setError(cause.message),
