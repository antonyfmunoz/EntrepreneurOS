...
function evaluateFormula(value: string, column: string, rows: Row[]) {
...
export function NativeSheetsStudio({ root, roleScopeKey, canExecute, canDecide }: {
  ...
  const selectedSheet = worksheets.find((item) => item.id === selectedSheetId) || worksheets[0];
  ...
  const createWorkbook = useMutation({
    ...
    onSuccess: async (result) => { setSelectedWorkbookId(result.object.id); setWorkbookTitle(""); await refresh(); },
