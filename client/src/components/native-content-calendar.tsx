...
import { CalendarDays, FileCheck2, Link2, Plus, RefreshCw, Save, Send } from "lucide-react";
...
export function NativeContentCalendar({ root, roleScopeKey, canExecute, canDecide }: {
  ...
  const [relationshipType, setRelationshipType] = useState("uses_brief");
  ...
  const graphQuery = useQuery<Json>({
    // This endpoint is already filtered by both tool entitlement and the
    // active role's record-visibility scope. A content planner never gets a
    // dropdown containing an inaccessible Finance, People, or Legal record.
    queryKey: [root, roleScopeKey, "native-content-calendar-graph"],
    queryFn: async () => (await apiRequest("GET", `${root}/instruments`)).json(),
  });
  const visibleTargets = useMemo(() => (graphQuery.data?.objects || [])
    .filter((item: Json) => item.id !== selectedContent?.id)
    .sort((left: Json, right: Json) => `${left.instrumentKey}:${left.title}`.localeCompare(`${right.instrumentKey}:${right.title}`)), [graphQuery.data?.objects, selectedContent?.id]);
  const selectedLinks = useMemo(() => (graphQuery.data?.links || []).filter((link: Json) => link.sourceObjectId === selectedContent?.id || link.targetObjectId === selectedContent?.id), [graphQuery.data?.links, selectedContent?.id]);
  ...
  const refresh = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: [root, roleScopeKey, "native-content-calendar"] }),
      queryClient.invalidateQueries({ queryKey: [root, roleScopeKey, "native-content-calendar-graph"] }),
    ]);
  };
  ...
  const linkContent = useMutation({
    mutationFn: async () => {
      if (!selectedContent) throw new Error("Choose a content item before linking it.");
      if (!linkTargetId) throw new Error("Choose an authorized supporting record.");
      return (await apiRequest("POST", `${root}/instrument-links`, {
        sourceObjectId: selectedContent.id,
        targetObjectId: linkTargetId,
        relationshipType: relationshipType.trim(),
        metadata: { purpose: "editorial_support" },
        idempotencyKey: commandKey("content-calendar-link"),
      })).json();
    },
    onSuccess: async () => {
      setLinkTargetId("");
