...
// A native dialer is a controlled outreach queue and outcome ledger. It does
// not assert that EOS placed a call: actual provider effects must be linked by
// a separately qualified provider execution or receipt.
export const eosOutreachSequences = pgTable("eos_outreach_sequences", {
  id: text("id").primaryKey(), companyId: integer("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), portfolioId: integer("portfolio_id").references(() => portfolios.id, { onDelete: "set null" }),
  sequenceKey: text("sequence_key").notNull(), title: text("title").notNull(), relationshipId: text("relationship_id").notNull().references(() => eosStakeholderRelationships.id, { onDelete: "restrict" }), commercialCaseId: text("commercial_case_id").references(() => eosCommercialCases.id, { onDelete: "set null" }),
  channel: text("channel").notNull(), state: text("state").notNull().default("draft"), ownerSeatId: text("owner_seat_id").notNull().references(() => eosSeats.id, { onDelete: "restrict" }), purpose: text("purpose").notNull(), script: text("script").notNull().default(""), consentBasis: text("consent_basis").notNull(), quietHours: text("quiet_hours").notNull().default(""), cadence: text("cadence").notNull().default(""), nextAttemptAt: timestamp("next_attempt_at", { withTimezone: true }),
  evidenceKeys: jsonb("evidence_keys").notNull().default([]), sourceAuthority: text("source_authority").notNull().default("native_eos"), classification: text("classification").notNull().default("internal"), recordedByUserId: text("recorded_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [
  uniqueIndex("eos_outreach_sequences_company_key_idx").on(table.companyId, table.sequenceKey), index("eos_outreach_sequences_owner_state_idx").on(table.ownerSeatId, table.state), index("eos_outreach_sequences_relationship_idx").on(table.relationshipId, table.updatedAt),
  check("eos_outreach_sequences_channel_check", sql`${table.channel} IN ('phone','email','sms','social','mixed','manual')`), check("eos_outreach_sequences_state_check", sql`${table.state} IN ('draft','active','paused','completed','cancelled')`), check("eos_outreach_sequences_classification_check", sql`${table.classification} IN ('public','internal','confidential','restricted')`), check("eos_outreach_sequences_authority_check", sql`${table.sourceAuthority} IN ('native_eos','notion_runtime','external_authoritative','reconciled')`), check("eos_outreach_sequences_evidence_check", sql`jsonb_typeof(${table.evidenceKeys}) = 'array'`),
]);

export const eosOutreachAttempts = pgTable("eos_outreach_attempts", {
  id: text("id").primaryKey(), companyId: integer("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), sequenceId: text("sequence_id").notNull().references(() => eosOutreachSequences.id, { onDelete: "cascade" }), ownerSeatId: text("owner_seat_id").notNull().references(() => eosSeats.id, { onDelete: "restrict" }),
  outcome: text("outcome").notNull(), note: text("note").notNull().default(""), attemptedAt: timestamp("attempted_at", { withTimezone: true }).notNull(), nextAttemptAt: timestamp("next_attempt_at", { withTimezone: true }), providerReceiptReference: text("provider_receipt_reference").notNull().default(""), recordedByUserId: text("recorded_by_user_id").notNull().references(() => users.id, { onDelete: "restrict" }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [
  index("eos_outreach_attempts_sequence_created_idx").on(table.sequenceId, table.createdAt), index("eos_outreach_attempts_company_owner_idx").on(table.companyId, table.ownerSeatId, table.createdAt),
  check("eos_outreach_attempts_outcome_check", sql`${table.outcome} IN ('planned','no_answer','voicemail','reached','meeting_booked','follow_up','not_interested','do_not_contact','invalid_contact')`),
]);
