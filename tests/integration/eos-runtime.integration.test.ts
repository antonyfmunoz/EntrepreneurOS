...
describe.skipIf(!databaseUrl)("EOS overlay HTTP lifecycle", () => {
  ...
  it("operates the native workflow, institutional-learning, stakeholder-portal, and conformance end-state controls", async () => {
    ...
    const handoff = await api.post(`/api/eos/companies/${companyId}/stakeholder-portals/${portal.body.id}/intake-forms/${intakeForm.body.id}/submissions/${intakeSubmission.id}/reviews/${intakeReview.body.id}/handoff`).send({ title: "Synthetic client launch preparation", priority: "high" }).expect(201);
    expect(handoff.body).toMatchObject({ workPacket: { status: "ready", classification: "confidential", source: "manual" } });
    ...
    expect(clientHandoffOutbox).toMatchObject({ aggregate_type: "stakeholder_portal_intake_handoff", aggregate_id: handoff.body.handoff.id, state: "dispatched", attempts: 1, payload: { stakeholderPortalId: portal.body.id, formObjectId: intakeForm.body.id, submissionObjectId: intakeSubmission.id, reviewObjectId: intakeReview.body.id, workPacketId: handoff.body.workPacket.id, targetSeatId: process.accountable_seat_id, priority: "high", classification: "confidential", verificationState: "unverified", externalEffectsExecuted: false } });
    ...
    if (clientHandoffSchedule) {
      ...
      expect(clientHandoffRun.body.runs.find((item: any) => item.id === clientHandoffOutbox.dispatched_run_ids[0])).toMatchObject({ companyId, processDefinitionId: process.id, ownerSeatId: process.accountable_seat_id, input: { fixture: "client-onboarding-handoff", _scheduleId: clientHandoffSchedule.body.id, _agentTrigger: { kind: "event", id: clientHandoffOutbox.id, eventType: "eos.stakeholder_portal.intake.handed_off.v1", payload: { stakeholderPortalId: portal.body.id, formObjectId: intakeForm.body.id, submissionObjectId: intakeSubmission.id, reviewObjectId: intakeReview.body.id, workPacketId: handoff.body.workPacket.id, targetSeatId: process.accountable_seat_id, priority: "high", classification: "confidential", verificationState: "unverified", externalEffectsExecuted: false } } } });
    }
