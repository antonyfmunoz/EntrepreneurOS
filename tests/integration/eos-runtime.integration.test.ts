...
describe.skipIf(!databaseUrl)("EOS overlay HTTP lifecycle", () => {
  ...
  it("operates canonical instruments through tenant-safe commands, versions, relationships, and immutable events", async () => {
    ...
    const pageWithFunnelTarget = await api.patch(`/api/eos/companies/${companyId}/instrument-objects/${page.body.object.id}`).send({ expectedVersion: 2, data: { publicPage: true, siteObjectId: site.body.object.id, headline: "A native EOS public page", supportingCopy: "Owned public copy without an external site builder.", primaryCtaLabel: "Open funnel", primaryCtaTarget: "funnel", primaryCtaTargetId: funnel.body.object.id, primaryCtaHref: "", path: "/fixture" }, idempotencyKey: "instrument:update:public-page-funnel-target" }).expect(200);
    ...
    expect(routedPublicPage.body).toMatchObject({ schemaVersion: "eos.public-page.v1", page: { id: page.body.object.id, siteName: "Fixture Studio", path: "/fixture", primaryCtaHref: `/f/${funnel.body.object.id}` } });
