...
describe.skipIf(!databaseUrl)("EOS overlay HTTP lifecycle", () => {
  ...
  it("operates canonical instruments through tenant-safe commands, versions, relationships, and immutable events", async () => {
    ...
    const customerRelationship = await api.patch(`/api/eos/companies/${companyId}/instrument-objects/${capturedRelationships[0].id}`).send({
      expectedVersion: capturedRelationships[0].version,
      data: { ...capturedRelationships[0].data, relationshipType: "customer" },
      idempotencyKey: "instrument:promote:public-intake-customer",
    }).expect(200);
    ...
    const nativeCustomerAccount = await api.post(`/api/eos/companies/${companyId}/customer-success/accounts`).send(nativeCustomerAccountPayload).expect(201);
    expect(nativeCustomerAccount.body).toMatchObject({ companyId, ownerSeatId: instrumentFounderSeat.id, lifecycleState: "active" });
    const customerSuccessAfter = await api.get(`/api/eos/companies/${companyId}/customer-success`).expect(200);
    expect(customerSuccessAfter.body.accounts).toEqual(expect.arrayContaining([
    ...
    const page = await api.post(`/api/eos/companies/${companyId}/instrument-objects`).send({
      ...
      data: { publicPage: true, siteObjectId: site.body.object.id, headline: "A native EOS public page", supportingCopy: "Owned public copy without an external site builder.", primaryCtaLabel: "Open intake", primaryCtaTarget: "capture_form", primaryCtaTargetId: captureForm.body.object.id, primaryCtaHref: "", path: "/fixture", sections: [{ id: "outcomes", kind: "outcomes", title: "Measurable outcomes", body: "", items: ["A governed native public page"] }] }, sourceReference: { authority: "native_eos", capability: "native_website_page" }, evidenceIds: [], idempotencyKey: "instrument:create:public-page",
    }).expect(201);
    ...
    expect(publicPage.body).toMatchObject({ schemaVersion: "eos.public-page.v2", page: { id: page.body.object.id, siteName: "Fixture Studio", headline: "A native EOS public page", primaryCtaHref: `/capture/${captureForm.body.object.id}`, sections: [{ id: "outcomes", title: "Measurable outcomes", items: ["A governed native public page"] }] } });
    expect(JSON.stringify(publicPage.body)).not.toMatch(/ownerSeatId|sourceReference|evidenceIds|policyDecision/i);
    const pageWithFunnelTarget = await api.patch(`/api/eos/companies/${companyId}/instrument-objects/${page.body.object.id}`).send({ expectedVersion: 2, data: { publicPage: true, siteObjectId: site.body.object.id, headline: "A native EOS public page", supportingCopy: "Owned public copy without an external site builder.", primaryCtaLabel: "Open funnel", primaryCtaTarget: "funnel", primaryCtaTargetId: funnel.body.object.id, primaryCtaHref: "", path: "/fixture", sections: [{ id: "steps", kind: "steps", title: "How it works", body: "", items: ["Request the review", "Receive the next step"] }] }, idempotencyKey: "instrument:update:public-page-funnel-target" }).expect(200);
    expect(pageWithFunnelTarget.body.object.version).toBe(3);
  ...
  it("applies only missing native tool baseline to an already-instantiated blueprint role", async () => {
    ...
    expect(instantiated.body.createdNativeAssetLinkIds).toHaveLength(16);
    ...
    expect(nativeAssetLinks).toEqual(expect.arrayContaining([
      expect.objectContaining({ sourceObjectId: starter("company-operating-brief").id, targetObjectId: starter("native-offer-catalog").id, relationshipType: "defines_offer", metadata: expect.objectContaining({ compilerStarter: true }) }),
      expect.objectContaining({ sourceObjectId: starter("native-offer-catalog").id, targetObjectId: starter("commercial-page").id, relationshipType: "presented_on", metadata: expect.objectContaining({ compilerStarter: true }) }),
      ...
      expect.objectContaining({ sourceObjectId: starter("finance-reconciliation-register").id, targetObjectId: starter("finance-control-worksheet").id, relationshipType: "reconciled_in", metadata: expect.objectContaining({ compilerStarter: true }) }),
    ]));
  ...
  it("keeps a product company’s native commercial assets with Brand & Growth", async () => {
    ...
    const nativeAssets = await sql<Array<{ ownerSeatId: string; objectKey: string }>>`
      SELECT owner_seat_id AS "ownerSeatId", object_key AS "objectKey"
      FROM eos_instrument_objects
  ...
  it("operates the native workflow, institutional-learning, stakeholder-portal, and conformance end-state controls", async () => {
    ...
    let clientHandoffSchedule: any = null;
    if (subject) {
      ...
      const lifecycleEventType = "eos.workflow_run.transitioned.v1";
      const lifecycleSchedule = await api.post(`/api/eos/companies/${companyId}/agent-schedules`).send({ scheduleKey: `fixture-lifecycle-event-${randomUUID()}`, name: "Workflow transition Role Agent", seatId: process.accountable_seat_id, authoritySubjectId: subject.id, processDefinitionId: process.id, triggerKind: "event", cadence: "event", eventTypes: [lifecycleEventType], executionMode: process.occupant_user_id ? "assisted" : "autonomous", inputTemplate: { fixture: "native-lifecycle" }, maxRunsPerDay: 2, evaluationRequired: true, classification: "confidential" }).expect(201);
      await api.patch(`/api/eos/companies/${companyId}/agent-schedules/${lifecycleSchedule.body.id}/state`).send({ expectedVersion: 1, state: "active", rationale: "Activate only after reviewing the native workflow transition event contract and its bounded Role Agent response." }).expect(200);
      const lifecycleSource = await api.post(`/api/eos/companies/${companyId}/workflow-runs`).send({ processDefinitionId: process.id, executionMode: "manual", idempotencyKey: `integration-lifecycle-source-${randomUUID()}`, input: { fixture: "lifecycle-source" }, classification: "confidential" }).expect(201);
      const lifecycleStarted = await api.post(`/api/eos/companies/${companyId}/workflow-runs/${lifecycleSource.body.id}/transition`).send({ expectedVersion: 1, action: "start", note: "Start a bounded native lifecycle source run so the configured Role Agent receives the committed transition event.", output: {}, evidenceIds: [], blocker: "" }).expect(200);
      expect(lifecycleStarted.body.agentEvent).toMatchObject({ state: "dispatched", matchingSchedules: 1 });
      const lifecycleState = await api.get(`/api/eos/companies/${companyId}/workflow-runtime`).expect(200);
      expect(lifecycleState.body.runs.find((item: any) => item.id === lifecycleStarted.body.agentEvent.runIds[0])).toMatchObject({ input: { fixture: "native-lifecycle", _scheduleId: lifecycleSchedule.body.id, _agentTrigger: { kind: "event", eventType: lifecycleEventType, payload: { workflowRunId: lifecycleSource.body.id, action: "start", fromState: "queued", toState: "running", externalEffectsExecuted: false } } } });
      const [outbox] = await sql<{ state: string; attempts: number; dispatched_run_ids: string[] }[]>`SELECT state, attempts, dispatched_run_ids FROM eos_agent_event_outbox WHERE id = ${lifecycleStarted.body.agentEvent.id}`;
      expect(outbox).toMatchObject({ state: "dispatched", attempts: 1, dispatched_run_ids: [lifecycleStarted.body.agentEvent.runIds[0]] });
      const [agentRunEventCount] = await sql<{ count: number }[]>`SELECT count(*)::int AS count FROM eos_agent_event_outbox WHERE aggregate_id = ${lifecycleStarted.body.agentEvent.runIds[0]}`;
      expect(agentRunEventCount.count).toBe(0);

      const approvalEventType = "eos.approval.decided.v1";
      const approvalSchedule = await api.post(`/api/eos/companies/${companyId}/agent-schedules`).send({ scheduleKey: `fixture-approval-event-${randomUUID()}`, name: "Approval decision Role Agent", seatId: process.accountable_seat_id, authoritySubjectId: subject.id, processDefinitionId: process.id, triggerKind: "event", cadence: "event", eventTypes: [approvalEventType], executionMode: process.occupant_user_id ? "assisted" : "autonomous", inputTemplate: { fixture: "native-approval" }, maxRunsPerDay: 2, evaluationRequired: true, classification: "confidential" }).expect(201);
      await api.patch(`/api/eos/companies/${companyId}/agent-schedules/${approvalSchedule.body.id}/state`).send({ expectedVersion: 1, state: "active", rationale: "Activate the bounded post-decision handoff only after binding it to the exact verified role and released process." }).expect(200);
      const approvalSource = await api.post(`/api/eos/companies/${companyId}/work-packets`).send({ title: "Native approval event fixture", objective: "Verify that a committed human approval can hand the next bounded action to a governed Role Agent.", requiresApproval: true, evidenceRequirements: [] }).expect(201);
      const approvalDecision = await api.post(`/api/eos/companies/${companyId}/approvals/${approvalSource.body.approvalId}/decide`).send({ decision: "approved", reason: "Approve the bounded native event fixture without any provider effect." }).expect(200);
      expect(approvalDecision.body.agentEvent).toMatchObject({ state: "dispatched", matchingSchedules: 1 });
      const approvalState = await api.get(`/api/eos/companies/${companyId}/workflow-runtime`).expect(200);
      expect(approvalState.body.runs.find((item: any) => item.id === approvalDecision.body.agentEvent.runIds[0])).toMatchObject({ input: { fixture: "native-approval", _scheduleId: approvalSchedule.body.id, _agentTrigger: { kind: "event", eventType: approvalEventType, payload: { approvalId: approvalSource.body.approvalId, workPacketId: approvalSource.body.id, decision: "approved", previousStatus: "pending", workPacketStatus: "ready", externalEffectsExecuted: false } } } });
      const [approvalOutbox] = await sql<{ aggregate_type: string; aggregate_id: string; state: string; attempts: number; dispatched_run_ids: string[] }[]>`SELECT aggregate_type, aggregate_id, state, attempts, dispatched_run_ids FROM eos_agent_event_outbox WHERE id = ${approvalDecision.body.agentEvent.id}`;
      expect(approvalOutbox).toMatchObject({ aggregate_type: "approval", aggregate_id: approvalSource.body.approvalId, state: "dispatched", attempts: 1, dispatched_run_ids: [approvalDecision.body.agentEvent.runIds[0]] });

      const instrumentEventType = "eos.instrument.object.transitioned.v1";
      const instrumentSchedule = await api.post(`/api/eos/companies/${companyId}/agent-schedules`).send({ scheduleKey: `fixture-instrument-event-${randomUUID()}`, name: "Native tool lifecycle Role Agent", seatId: process.accountable_seat_id, authoritySubjectId: subject.id, processDefinitionId: process.id, triggerKind: "event", cadence: "event", eventTypes: [instrumentEventType], eventFilter: { all: [{ path: "instrumentKey", equals: "docs" }, { path: "toState", equals: "active" }] }, executionMode: process.occupant_user_id ? "assisted" : "autonomous", inputTemplate: { fixture: "native-instrument" }, maxRunsPerDay: 2, evaluationRequired: true, classification: "confidential" }).expect(201);
      await api.patch(`/api/eos/companies/${companyId}/agent-schedules/${instrumentSchedule.body.id}/state`).send({ expectedVersion: 1, state: "active", rationale: "Activate the bounded tool-lifecycle handoff only after binding it to the exact verified role and released process." }).expect(200);
      const instrumentSource = await api.post(`/api/eos/companies/${companyId}/instrument-objects`).send({ instrumentKey: "docs", objectType: "document", objectKey: `document:agent-event-${randomUUID()}`, title: "Native instrument event fixture", summary: "A bounded native document lifecycle source for Role Agent automation verification.", classification: "confidential", visibility: "organization", data: { body: "Synthetic, non-production document content.", format: "markdown" }, sourceReference: { kind: "native_eos" }, evidenceIds: [], idempotencyKey: `instrument:create:agent-event-${randomUUID()}` }).expect(201);
      const instrumentTransition = await api.post(`/api/eos/companies/${companyId}/instrument-objects/${instrumentSource.body.object.id}/transitions`).send({ expectedVersion: 1, state: "active", rationale: "Activate this bounded native document and hand its committed lifecycle change to the governed Role Agent.", evidenceIds: [], idempotencyKey: `instrument:transition:agent-event-${randomUUID()}` }).expect(200);
      expect(instrumentTransition.body.agentEvent).toMatchObject({ state: "dispatched", matchingSchedules: 1 });
      const instrumentState = await api.get(`/api/eos/companies/${companyId}/workflow-runtime`).expect(200);
      expect(instrumentState.body.runs.find((item: any) => item.id === instrumentTransition.body.agentEvent.runIds[0])).toMatchObject({ input: { fixture: "native-instrument", _scheduleId: instrumentSchedule.body.id, _agentTrigger: { kind: "event", eventType: instrumentEventType, payload: { instrumentKey: "docs", objectId: instrumentSource.body.object.id, objectType: "document", fromState: "draft", toState: "active", objectVersion: 2, externalEffectsExecuted: false } } } });
      const [instrumentOutbox] = await sql<{ aggregate_type: string; aggregate_id: string; state: string; attempts: number; dispatched_run_ids: string[] }[]>`SELECT aggregate_type, aggregate_id, state, attempts, dispatched_run_ids FROM eos_agent_event_outbox WHERE id = ${instrumentTransition.body.agentEvent.id}`;
      expect(instrumentOutbox).toMatchObject({ aggregate_type: "instrument_object", aggregate_id: instrumentSource.body.object.id, state: "dispatched", attempts: 1, dispatched_run_ids: [instrumentTransition.body.agentEvent.runIds[0]] });

      const leadEventType = "eos.crm.consented_lead_recorded.v1";
      const leadSchedule = await api.post(`/api/eos/companies/${companyId}/agent-schedules`).send({ scheduleKey: `fixture-consented-lead-${randomUUID()}`, name: "Consented lead Role Agent", seatId: process.accountable_seat_id, authoritySubjectId: subject.id, processDefinitionId: process.id, triggerKind: "event", cadence: "event", eventTypes: [leadEventType], executionMode: process.occupant_user_id ? "assisted" : "autonomous", inputTemplate: { fixture: "consented-native-lead" }, maxRunsPerDay: 2, evaluationRequired: true, classification: "confidential" }).expect(201);
      await api.patch(`/api/eos/companies/${companyId}/agent-schedules/${leadSchedule.body.id}/state`).send({ expectedVersion: 1, state: "active", rationale: "Activate the post-consent CRM handoff only after binding it to the exact verified role and released process." }).expect(200);
      const leadForm = await api.post(`/api/eos/companies/${companyId}/instrument-objects`).send({ instrumentKey: "forms", objectType: "form", objectKey: `form:agent-lead-${randomUUID()}`, title: "Native agent lead fixture", summary: "Synthetic consented intake for governed Role Agent dispatch.", classification: "confidential", visibility: "organization", data: { publicCapture: true, questions: [{ id: "email", label: "Work email", type: "email", required: true, options: [] }], consentVersion: "agent-event-v1", consentLabel: "I consent to be contacted about this request.", confirmationMessage: "Recorded." }, sourceReference: { authority: "native_eos", capability: "native_agent_lead_fixture" }, evidenceIds: [], idempotencyKey: `instrument:create:agent-lead-${randomUUID()}` }).expect(201);
      const leadFormTransition = await api.post(`/api/eos/companies/${companyId}/instrument-objects/${leadForm.body.object.id}/transitions`).send({ expectedVersion: 1, state: "active", rationale: "Founder publishes the synthetic consented lead fixture after its native controls are reviewed.", evidenceIds: [], idempotencyKey: `instrument:transition:agent-lead-${randomUUID()}` }).expect(200);
      expect(leadFormTransition.body.agentEvent).toMatchObject({ state: "dispatched", matchingSchedules: 0 });
      const leadSubmission = await api.post(`/api/public/lead-forms/${leadForm.body.object.id}/submissions`).send({ answers: { email: "role-agent-lead@example.test" }, consent: true }).expect(201);
      expect(leadSubmission.body).toMatchObject({ accepted: true });
      const [leadOutbox] = await sql<{ id: string; aggregate_type: string; aggregate_id: string; state: string; attempts: number; dispatched_run_ids: string[]; payload: Record<string, unknown> }[]>`SELECT id, aggregate_type, aggregate_id, state, attempts, dispatched_run_ids, payload FROM eos_agent_event_outbox WHERE company_id = ${companyId} AND event_type = ${leadEventType} ORDER BY created_at DESC LIMIT 1`;
      expect(leadOutbox).toMatchObject({ aggregate_type: "lead_submission", state: "dispatched", attempts: 1 });
      expect(leadOutbox.dispatched_run_ids).toHaveLength(1);
      expect(leadOutbox.payload).toMatchObject({ formObjectId: leadForm.body.object.id, consentRecorded: true, consentVersion: "agent-event-v1", externalEffectsExecuted: false });
      expect(JSON.stringify(leadOutbox.payload)).not.toContain("role-agent-lead@example.test");
      const leadState = await api.get(`/api/eos/companies/${companyId}/workflow-runtime`).expect(200);
      expect(leadState.body.runs.find((item: any) => item.id === leadOutbox.dispatched_run_ids[0])).toMatchObject({ input: { fixture: "consented-native-lead", _scheduleId: leadSchedule.body.id, _agentTrigger: { kind: "event", id: leadOutbox.id, eventType: leadEventType, payload: { formObjectId: leadForm.body.object.id, consentRecorded: true, consentVersion: "agent-event-v1", externalEffectsExecuted: false } } } });

      const clientHandoffEventType = "eos.stakeholder_portal.intake.handed_off.v1";
      clientHandoffSchedule = await api.post(`/api/eos/companies/${companyId}/agent-schedules`).send({ scheduleKey: `fixture-client-handoff-${randomUUID()}`, name: "Client onboarding handoff Role Agent", seatId: process.accountable_seat_id, authoritySubjectId: subject.id, processDefinitionId: process.id, triggerKind: "event", cadence: "event", eventTypes: [clientHandoffEventType], eventFilter: { all: [{ path: "targetSeatId", equals: process.accountable_seat_id }, { path: "verificationState", equals: "unverified" }] }, executionMode: process.occupant_user_id ? "assisted" : "autonomous", inputTemplate: { fixture: "client-onboarding-handoff" }, maxRunsPerDay: 2, evaluationRequired: true, classification: "confidential" }).expect(201);
      await api.patch(`/api/eos/companies/${companyId}/agent-schedules/${clientHandoffSchedule.body.id}/state`).send({ expectedVersion: 1, state: "active", rationale: "Activate the bounded client onboarding response only for the exact accountable role after a reviewed native handoff event." }).expect(200);
    }
    ...
    const portal = await api.post(`/api/eos/companies/${companyId}/stakeholder-portals`).send({ portalKey: `client-fixture-${randomUUID()}`, name: "Client fixture workspace", portalType: "client", ownerSeatId: process.accountable_seat_id, visibleSections: ["updates"], activationRequirements: ["Verify the intended recipient"] }).expect(201);
    ...
    const intakeForm = await api.post(`/api/eos/companies/${companyId}/stakeholder-portals/${portal.body.id}/intake-forms`).send({ formKey: "client-onboarding", title: "Synthetic client onboarding", summary: "Collect bounded synthetic onboarding context for the private-workspace lifecycle qualification.", questions: [{ id: "launch_outcome", label: "Launch outcome", type: "long_text", required: true, options: [] }, { id: "primary_email", label: "Primary email", type: "email", required: true, options: [] }], confirmationMessage: "Synthetic onboarding input recorded for accountable review." }).expect(201);
    await api.patch(`/api/eos/companies/${companyId}/stakeholder-portals/${portal.body.id}/intake-forms/${intakeForm.body.id}`).send({ expectedVersion: 1, state: "active", rationale: "Activate the synthetic client onboarding form only after its private workspace is active and its review boundary is understood." }).expect(200);
    ...
    expect(publicView.body.intakeForms).toEqual(expect.arrayContaining([expect.objectContaining({ id: intakeForm.body.id, submitted: false })]));
    const submittedIntake = await api.post(`/api/public/stakeholder-portals/${grant.body.token}/intake-forms/${intakeForm.body.id}/submissions`).send({ answers: { launch_outcome: "Complete the synthetic client launch without asserting any provider or customer effect.", primary_email: "synthetic-client-onboarding@example.test" }, acknowledgement: true, website: "" }).expect(201);
    expect(submittedIntake.body).toMatchObject({ accepted: true, verificationState: "unverified" });
    const publicAfterSubmission = await api.get(`/api/public/stakeholder-portals/${grant.body.token}`).expect(200);
    expect(publicAfterSubmission.body.intakeForms).toEqual(expect.arrayContaining([expect.objectContaining({ id: intakeForm.body.id, submitted: true })]));
    expect(JSON.stringify(publicAfterSubmission.body)).not.toContain("synthetic-client-onboarding@example.test");
    const internalIntake = await api.get(`/api/eos/companies/${companyId}/stakeholder-portals/${portal.body.id}/intake-forms/${intakeForm.body.id}/submissions`).expect(200);
    const intakeSubmission = internalIntake.body.submissions[0];
    expect(intakeSubmission).toMatchObject({ verificationState: "unverified", review: null, responses: { primary_email: "synthetic-client-onboarding@example.test" } });
    const intakeReview = await api.post(`/api/eos/companies/${companyId}/stakeholder-portals/${portal.body.id}/intake-forms/${intakeForm.body.id}/submissions/${intakeSubmission.id}/reviews`).send({ disposition: "action_required", reviewerSummary: "The founder reviewed the synthetic client input and identified an internal launch preparation action while retaining every client-supplied answer as unverified.", nextAction: "Create the bounded native launch-preparation Work Packet for the accountable client-workspace owner." }).expect(201);
    expect(intakeReview.body.boundary).toContain("remain unverified");
    await api.post(`/api/eos/companies/${companyId}/stakeholder-portals/${portal.body.id}/intake-forms/${intakeForm.body.id}/submissions/${intakeSubmission.id}/reviews`).send({ disposition: "acknowledged", reviewerSummary: "A duplicate review must not replace the accountable disposition or erase its immutable client-input boundary.", nextAction: "Preserve the original review." }).expect(409).expect(({ body }) => expect(body.code).toBe("stakeholder_portal_intake_already_reviewed"));
    const reviewedIntake = await api.get(`/api/eos/companies/${companyId}/stakeholder-portals/${portal.body.id}/intake-forms/${intakeForm.body.id}/submissions`).expect(200);
    const reviewedSubmission = reviewedIntake.body.submissions[0];
    expect(reviewedSubmission).toMatchObject({ verificationState: "unverified", review: { id: intakeReview.body.id, disposition: "action_required", handoff: null } });
    const handoff = await api.post(`/api/eos/companies/${companyId}/stakeholder-portals/${portal.body.id}/intake-forms/${intakeForm.body.id}/submissions/${intakeSubmission.id}/reviews/${intakeReview.body.id}/handoff`).send({ title: "Synthetic client launch preparation", priority: "high", processDefinitionId: process.id }).expect(201);
    expect(handoff.body).toMatchObject({ handoff: { processDefinitionId: process.id }, workPacket: { status: "ready", classification: "confidential", source: "manual", processDefinitionId: process.id } });
    expect(handoff.body.boundary).toContain("did not perform a provider");
    expect(handoff.body.agentEvent).toMatchObject({ eventType: "eos.stakeholder_portal.intake.handed_off.v1", state: "dispatched", runIds: clientHandoffSchedule ? [expect.any(String)] : [] });
    const [clientHandoffOutbox] = await sql<{ id: string; aggregate_type: string; aggregate_id: string; state: string; attempts: number; dispatched_run_ids: string[]; payload: Record<string, unknown> }[]>`SELECT id, aggregate_type, aggregate_id, state, attempts, dispatched_run_ids, payload FROM eos_agent_event_outbox WHERE company_id = ${companyId} AND event_type = 'eos.stakeholder_portal.intake.handed_off.v1' ORDER BY created_at DESC LIMIT 1`;
    expect(clientHandoffOutbox).toMatchObject({ aggregate_type: "stakeholder_portal_intake_handoff", aggregate_id: handoff.body.handoff.id, state: "dispatched", attempts: 1, payload: { stakeholderPortalId: portal.body.id, formObjectId: intakeForm.body.id, submissionObjectId: intakeSubmission.id, reviewObjectId: intakeReview.body.id, workPacketId: handoff.body.workPacket.id, targetSeatId: process.accountable_seat_id, targetProcessDefinitionId: process.id, priority: "high", classification: "confidential", verificationState: "unverified", externalEffectsExecuted: false } });
    expect(JSON.stringify(clientHandoffOutbox.payload)).not.toContain("synthetic-client-onboarding@example.test");
    if (clientHandoffSchedule) {
      expect(clientHandoffOutbox.dispatched_run_ids).toHaveLength(1);
      const clientHandoffRun = await api.get(`/api/eos/companies/${companyId}/workflow-runtime`).expect(200);
      expect(clientHandoffRun.body.runs.find((item: any) => item.id === clientHandoffOutbox.dispatched_run_ids[0])).toMatchObject({ companyId, processDefinitionId: process.id, workPacketId: handoff.body.workPacket.id, ownerSeatId: process.accountable_seat_id, input: { fixture: "client-onboarding-handoff", _scheduleId: clientHandoffSchedule.body.id, _agentTrigger: { kind: "event", id: clientHandoffOutbox.id, eventType: "eos.stakeholder_portal.intake.handed_off.v1", payload: { stakeholderPortalId: portal.body.id, formObjectId: intakeForm.body.id, submissionObjectId: intakeSubmission.id, reviewObjectId: intakeReview.body.id, workPacketId: handoff.body.workPacket.id, targetSeatId: process.accountable_seat_id, targetProcessDefinitionId: process.id, priority: "high", classification: "confidential", verificationState: "unverified", externalEffectsExecuted: false } } } });
    }
